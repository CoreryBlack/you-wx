...
function normalizeMiniProgramFilename(filename, inputDir) {
    // 如果 filename 为相对路径（例如来自 emitted fileName），去除前导 ../ 后直接处理
    if (!inputDir || !path_1.default.isAbsolute(filename)) {
        const sanitized = filename.replace(/^(?:\.\.\/)+/, '');
        return normalizeNodeModules(sanitized);
    }
    // 距离 inputDir 的相对路径可能包含前导 ../，这些在作为小程序资源名时会被视为非法相对路径。
    // 在此去除多余的 ../ 前缀以确保返回的资源名为合法的相对路径片段。
    let rel = path_1.default.relative(inputDir, filename);
    rel = rel.replace(/^(?:\.\.\/)+/, '');
    return normalizeNodeModules(rel);
}
